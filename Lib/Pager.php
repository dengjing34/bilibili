...
class Pager {
    ...
    public static function showPage($total, $pageSize = null) {
        ...
        if (!empty($queryString)) {
            ...
            if (isset($params['_rp_'])) unset($params['_rp_']);
        ...
        $firstLink = $currPage == 1 ? '<li class="disabled"><span>1</span></li>' : "<li><a href=\"?{$otherParmas}page=1\">1</a><li> ";
        $prevPage = $currPage == 1 ? 1 : $currPage - 1;
        $prevLink = $currPage == 1 ? null : "<li><a href=\"?{$otherParmas}page={$prevPage}\">&laquo;</a></li> ";
        $numberLink = $lastLink = null;
        if ($currPage - self::$numberLink > 2) $numberLink .= '<li class="disabled"><span>...</span></li>';
        for ($i = $currPage - self::$numberLink; $i <= $currPage + self::$numberLink; $i++) {
            if ($i > 1 && $i< $pageMax) {
                if ($i == $currPage) $numberLink .= "<li class=\"disabled\"><span>{$i}</span></li> ";
                else $numberLink .= "<li><a href=\"?{$otherParmas}page={$i}\">{$i}</a></li> ";
            }
        }
        if ($currPage + self::$numberLink < $pageMax - 1) $numberLink .= '<li class="disabled"><span>...</span></li>';
        $nextPage = $currPage < $pageMax ? $currPage + 1 : $pageMax;
        $nextLink = $currPage == $pageMax ? null : "<li><a href=\"?{$otherParmas}page={$nextPage}\">&raquo;</a></li> ";
        if ($pageMax != 1) {
            $lastLink = $currPage == $pageMax ? "<li class=\"disabled\"><span>{$pageMax}</span></li>" : "<li><a href=\"?{$otherParmas}page={$pageMax}\">{$pageMax}</a></li> ";
        }
        $ext = self::$pagerExt == true ? "<li class=\"disabled\"><span>当前{$currPage}/{$pageMax}页 {$pageSize}条/页 共{$total}条记录</span><li>" : '';
        //$result .= $firstLink . $prevLink . $nextLink . $lastLink . $ext;
        $result .= '<ul>' . $prevLink . $firstLink . $numberLink . $lastLink . $nextLink . $ext . '</ul>';
        return $result;
    }
